<template>
  <div class="player" v-show="playList.length > 0">
    ...
    <play-list ref="playListPanel"></play-list>
  </div>
</template>
...
import {getSongAddr} from 'api/singer'
...
export default {
  ...
  methods:{
    ...
    _getSongAddr() {
      getSongAddr(this.currentSong.mid).then((res) => {
        if (res.url) {
          this.$refs.audio.src=res.url
          this.$refs.audio.play()
        }else{
          console.log('get song.url err:')
          console.log(res)
        }
      })
    },
  ...
  watch: {
    currentSong(newSong, olderSong) {
      if(!newSong.id) {
        return
      }
      if(newSong.id === olderSong.id) {
        return
      }
      if (this.currentLyric) {
        this.currentLyric.stop()
