      ...
      <div class="list-wrapper" @click.stop>
        <div class="list-header">
          <h1 class="title">
            ...
            <span class="text">{{playModeDesc()}}</span>
        ...
        <Scroll ref="scroll" :data="sequenceList" class="list-content">
          <transition-group name="list" tag="ul">
            <li v-for="(song, index) in sequenceList"
                :key="song.id"
                class="item"
                ref="listItem"
            >
              <i class="current" :class="{'icon-play':song.id===currentSong.id}"></i>
              <span class="text" @click="selectSong(song,index)">{{index}}.{{song.name}}</span>
              <span class="like">
...
export default {
  ...
  methods:{
    ...
    deleteOne(song) {
      ...
      //this.scollToCurrentSong(this.currentSong)
      if(!this.playList.length) {
        this.hide()
      }
    },
    scollToCurrentSong(current) {
  ...
  watch: {
    currentSong(newSong,oldSong) {
      ...
      this.scollToCurrentSong(newSong)
    }
  },
  components: {
    Scroll
